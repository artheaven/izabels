...
export default function PackagingTypesPage() {
  ...
  const loadTypes = async () => {
    try {
      ...
      setTypes(response.data.categories.filter((c: any) => c.type === 'PACKAGING'));
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingType) {
        await adminApi.updateCategory(editingType.id, typeData);
      } else {
        await adminApi.createCategory(typeData);
      }
      ...
      alert(error.response?.data?.error || 'Ошибка при сохранении типа');
    }
  };
  ...
  const handleEdit = (type: any) => {
    setEditingType(type);
    setFormData({
      name_bg: type.translations.find((t: any) => t.language === 'bg')?.name || '',
      name_ru: type.translations.find((t: any) => t.language === 'ru')?.name || '',
      slug: type.slug,
    });
    setShowForm(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Сигурни ли сте, че искате да изтриете този тип?')) return;
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <h1 className="text-3xl font-bold">Типове опаковка</h1>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h2 className="text-xl font-bold mb-4">
            {editingType ? 'Редактиране на тип' : 'Нов тип'}
          </h2>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {types.map((type) => {
              const nameBg = type.translations.find((t: any) => t.language === 'bg')?.name || '-';
              const nameRu = type.translations.find((t: any) => t.language === 'ru')?.name || '-';
              return (
